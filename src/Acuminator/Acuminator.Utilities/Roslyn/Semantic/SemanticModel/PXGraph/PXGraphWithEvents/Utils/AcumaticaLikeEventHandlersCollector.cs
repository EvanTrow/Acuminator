...
{
	/// <summary>
	/// Event handlers collector that attempts to collect event handlers in the same order as Acumatica runtime.
	/// </summary>
	/// <remarks>
	/// This type is not used currently. First of all, it is not possible to collect event handlers precisely in the same order as Acumatica runtime<br/>
	/// because of exteral customizations loaded at runtime and custom ordering that can be applied by Acumatica runtime.<br/>
	/// These two factors make the order of loaded graph extension types effectively unpredictable at compile time.<br/>
	/// <br/><br/>
	/// The type is kept for possible future use due to some work already done in it for the event handlers collection.<br/>
	/// While precise order of event handlers is unpredictable, it is still possible to collect in the order close to what Acumatica does.<br/>
	/// Such collection may detect double execution of event handlers, or other problems. On the other hand, there are possible other ways to detect such problems.<br/>
	/// </remarks>
	file class AcumaticaLikeEventHandlersCollector
	{
		...
		public AcumaticaLikeEventHandlersCollector(PXGraphEventSemanticModel graphEventSemanticModel, PXContext context)
		{
			_pxContext = context;
