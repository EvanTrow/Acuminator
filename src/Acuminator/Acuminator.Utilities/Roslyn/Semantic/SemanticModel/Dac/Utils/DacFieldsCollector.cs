	...
	private static DacFieldInfo CreateDacFieldInfoWhenThereIsOneBqlField(DacBqlFieldInfo bqlFieldInfo, List<DacPropertyInfo>? propertyInfos)
	{
		switch (propertyInfos?.Count)
		{
			...
			default:
				...
				var propertyInfo = propertyInfos.FirstOrDefault(p => p.HasAcumaticaAttributesEffective &&
								   ...
								   propertyInfos.FirstOrDefault(p => p.HasAcumaticaAttributesEffective) ?? 
								   propertyInfos[0];
			...
			default:
				{
					var suitableFieldPropertyPairs = (from bqlFieldInfo in bqlFieldInfos
													  join propertyInfo in propertyInfos.Where(property => property.HasAcumaticaAttributesEffective)
					...
					if (suitableFieldPropertyPairs.Count == 0)
					{
						var suitablePropertyInfo = propertyInfos.FirstOrDefault(p => p.HasAcumaticaAttributesEffective) ?? 
												   propertyInfos[0];
