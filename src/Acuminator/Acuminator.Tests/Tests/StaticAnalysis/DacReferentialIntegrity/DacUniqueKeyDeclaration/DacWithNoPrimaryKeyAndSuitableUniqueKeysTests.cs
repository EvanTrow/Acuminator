...
namespace Acuminator.Tests.Tests.StaticAnalysis.DacReferentialIntegrity
{
	public class DacWithNoPrimaryKeyAndSuitableUniqueKeysTests : CodeFixVerifier
	{
		...
		public Task Dac_NoPK_SingleUniqueKey(string source) =>
		...
		public Task Dac_NoPK_TwoUniqueKeys(string source) =>
			VerifyCSharpDiagnosticAsync(source,
				Descriptors.PX1036_WrongDacPrimaryKeyName.CreateFor(
					location: (Line: 9, Column: 16), extraLocation: (Line: 6, Column: 2)));
		...
		public Task Dac_NoPK_MultipleUniqueKeys(string source) =>
			VerifyCSharpDiagnosticAsync(source,
				Descriptors.PX1036_WrongDacPrimaryKeyName.CreateFor(
		...
		public Task TurnUniqueKeyIntoPrimaryKey_SinqleKey_VerifyCodeFix(string actual, string expected) =>
		...
		public Task TurnUniqueKeyIntoPrimaryKey_TwoKeys_VerifyCodeFix(string actual, string expected) =>
			VerifyCSharpFixAsync(actual, expected);

		[Theory]
		[EmbeddedFileData(@"TurnUniqueKeyIntoPrimaryKey\INUnit_MultipleUniqueKeys_NoPK.cs", 
						  @"TurnUniqueKeyIntoPrimaryKey\INUnit_MultipleUniqueKeys_NoPK_Expected.cs")]
		public Task TurnUniqueKeyIntoPrimaryKey_MultipleUniqueKeys_VerifyCodeFix(string actual, string expected) =>
			VerifyCSharpFixAsync(actual, expected);
	}
}
