...
namespace Acuminator.Tests.Tests.StaticAnalysis.ThrowingExceptions
{
	public class ThrowingExceptionsInLongRunningOperationTests : DiagnosticVerifier
	{
		...
		public Task LongOperationStart_ReportsDiagnostic(string source) =>
			VerifyCSharpDiagnosticAsync(source, 
				Descriptors.PX1086_ThrowingSetupNotEnteredExceptionInLongRunningOperation.CreateFor(21, 13));

		[Theory]
		[EmbeddedFileData(@"LongOperations\ParametersDelegate_Bad.cs")]
		public Task ParametersDelegate_ReportsDiagnostic(string source) =>
			VerifyCSharpDiagnosticAsync(source,
		...
		public Task ProcessDelegate_ReportsDiagnostic(string source) =>
			VerifyCSharpDiagnosticAsync(source,
		...
		public Task FinallyProcessDelegate_ReportsDiagnostic(string source) =>
			VerifyCSharpDiagnosticAsync(source,
		...
		public Task LongOperationStart_WithoutPXSetupNotEnteredException_DoesntReportDiagnostic(string source) =>
			VerifyCSharpDiagnosticAsync(source);

		[Theory]
		[EmbeddedFileData(@"LongOperations\ParametersDelegate_Good.cs")]
		public Task ParametersDelegate_WithoutPXSetupNotEnteredException_DoesntReportDiagnostic(string source) =>
			VerifyCSharpDiagnosticAsync(source);

		[Theory]
		[EmbeddedFileData(@"LongOperations\ProcessDelegate_Good.cs")]
		public Task ProcessDelegate_WithoutPXSetupNotEnteredException_DoesntReportDiagnostic(string source) =>
			VerifyCSharpDiagnosticAsync(source);

		[Theory]
		[EmbeddedFileData(@"LongOperations\FinallyProcessDelegate_Good.cs")]
		public Task FinallyProcessDelegate_WithoutPXSetupNotEnteredException_DoesntReportDiagnostic(string source) =>
			VerifyCSharpDiagnosticAsync(source);
	}
}
