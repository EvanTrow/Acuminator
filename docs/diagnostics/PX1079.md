...
The code fix does not support methods with signatures that accept input or return result by reference instead of by value:
* methods with `ref`, `out`, `in`, or `ref readonly` parameters.
* methods with `ref` and `ref readonly` return types.
