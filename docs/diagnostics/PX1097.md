...
The code fix for the PX1097 diagnostic will change the accessibility of the method with the `PXOverride` attribute to `public` and remove any `virtual` modifiers from the method declaration.
The code fix cannot be applied to methods with the `override` modifier, because such change would break the compatibility of the method with the base method it overrides. In this case, the code fix will not be available.
...
Such change will introduce a compilation error since only abstract methods can be declared without a body.
