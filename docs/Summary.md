...
| [PX1120](diagnostics/PX1120.md) | Incorrect work with `Task` types in the Acumatica asynchronous code. You should not store `Task` instance in a local variable or parameter, `Task`-typed expressions should be awaited, and method returning a `Task`-typed expression should have `Task` type as its return type. | Warning | Unavailable |
